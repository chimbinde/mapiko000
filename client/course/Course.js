        ...
        <div className={classes.root}>
              <Card className={classes.card}>
                <CardHeader
                  title={course.name}
                  subheader={<div>
                        <Link to={"/user/"+course.instructor._id} className={classes.sub}>De {course.instructor.name}</Link>
             ...
             {auth.isAuthenticated().user && auth.isAuthenticated().user._id == course.instructor._id &&
                ...
                {!course.published ? (<>
                  ...
                  <Button color="primary" variant="outlined">Publicado</Button>
                )}
                </span>)
             }
                {course.published && (<div>
                  <span className={classes.statSpan}><PeopleIcon /> {stats.totalEnrolled} Inscrito </span>
                  <span className={classes.statSpan}><CompletedIcon/> {stats.totalCompleted} Finalisado </span>
                  </div>
                  )}
              ...
              {course.published && <div className={classes.enroll}><Enroll courseId={course._id}/></div>} 
                ...
                <CardHeader
                  ...
                  subheader={<Typography variant="body1" className={classes.subheading}>{course.lessons && course.lessons.length} Aulas</Typography>}
                  action={
             auth.isAuthenticated().user && auth.isAuthenticated().user._id == course.instructor._id && !course.published &&
              ...
              <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
                <DialogTitle id="form-dialog-title">Publicar Curso</DialogTitle>
                <DialogContent>
                  <Typography variant="body1">Ao Publicar seu curso fará com que este seja visível para estudantes inscritos. </Typography><Typography variant="body1">Certifique que todas aulas foram inseridas e estāo prontas para serem publicadas.</Typography></DialogContent>
                <DialogActions>
