...
export default function Enrollment ({match}) {
  ...
  const markComplete = () => {
    ...
    return (
      ...
      <List>
          <ListItem>
        <ListItemText primary={<div className={classes.progress}><span>{totalComplete}</span> fora de <span>{enrollment.lessonStatus.length}</span> completo</div>} />
          </ListItem>
      </List>
    </Drawer>
              {values.drawer == - 1 && 
              <Card className={classes.card}>
                <CardHeader
                  ...
                  subheader={<div>
                        <Link to={"/user/"+enrollment.course.instructor._id} className={classes.sub}>De {enrollment.course.instructor.name}</Link>
                ...
                <div>
                <CardHeader
                  title={<Typography variant="h6" className={classes.subheading}>Aulas</Typography>
                }
                  subheader={<Typography variant="body1" className={classes.subheading}>{enrollment.course.lessons && enrollment.course.lessons.length} aulas</Typography>}
                  action={
             ...
             <Card className={classes.card}>
                <CardHeader
                  ...
                  action={<Button onClick={markComplete} variant={enrollment.lessonStatus[values.drawer].complete? 'contained' : 'outlined'} color="secondary">{enrollment.lessonStatus[values.drawer].complete? "Completo" : "Marcar como completo"}</Button>} />
                  <CardContent> 
                      <Typography variant="body1" className={classes.para}>{enrollment.course.lessons[values.drawer].content}</Typography>
                  </CardContent>
                  <CardActions>
                    <a href={enrollment.course.lessons[values.drawer].resource_url}><Button variant="contained" color="primary">Endereço para recurso</Button></a>
                </CardActions>
                </Card></>)}
        </div>)
}